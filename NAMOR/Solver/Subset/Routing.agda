...
module NAMOR.Solver.Subset.Routing where
...
open import NAMOR.BoolReflect using (and-true-l; and-true-r)
open import NAMOR.List.Any renaming (here to hereA; there to thereA)
open import NAMOR.List.Membership as Mem using (_∈_; ∈-++⁺ˡ; ∈-++⁺ʳ)
...
open import NAMOR.Solver.Subset.Core
open import NAMOR.Solver.Subset.Normal

pattern there p = thereA p
...
module Routing {A : Type ℓ} (_≟_ : Discrete A) where
  ...
  elemInList-sound x (y ∷ ys) p = helper (x ≟ℕ y) refl p
    where
      ...
      helper true eq _ = subst (_∈ (y ∷ ys)) (sym (≟ℕ-sound x y eq)) (hereA refl)
  ...
  allIn-sound [] rs1 p x ()
  allIn-sound (r ∷ rs2) rs1 p x (hereA x≡r) =
    subst (_∈ rs1) (sym x≡r) (elemInList-sound r rs1 (and-true-l p))
  ...
  mem-removeIndices-intro (r ∷ rs) xs ρe y yIn notIn =
    ...
    where
       ...
       yNotInR v eq = notIn r (hereA refl) v eq
  ...
  not-eq-from-removeIndices : ∀ (rs : List ℕ) (xs : List A) (ρe : List A) (y : A)
                            → y ∈ removeIndices rs xs ρe
                            → ∀ k → k ∈ rs → ∀ v → lookupElem ρe k ≡ just v → y ≢ v
  not-eq-from-removeIndices (r ∷ rs) xs ρe y yIn k (hereA k≡r) v eq =
     -- k = r. y ∈ removeIndices rs (evalRem r xs)
     -- y ∈ evalRem r ... -> y != val(r)
     let yInEval = mem-removeIndices-subset rs (evalRem r xs ρe) ρe y yIn
         eqr : lookupElem ρe r ≡ just v
         eqr = subst (λ z → lookupElem ρe z ≡ just v) k≡r eq
     in helper (lookupElem ρe r) refl yInEval v eqr
     where
  ...
  atom-in-nf-⊆ : ∀ (b : Atom) (rhs : NormalForm) (ρ : Env)
               → b ∈ rhs
               → ⟦ b ⟧atom ρ ⊆ ⟦ rhs ⟧nf ρ
  atom-in-nf-⊆ b (b' ∷ bs) ρ (hereA b≡b') y yIn =
    mem-++-l (subst (λ a → y ∈ ⟦ a ⟧atom ρ) b≡b' yIn)
  ...
  findRoute-aux a (b' ∷ bs) b p = helper (atomCanRouteTo a b') refl
    where
      ...
      helper true eq =
        ...
        in (subst (_∈ (b' ∷ bs)) b'≡b (hereA refl)) , subst (λ x → atomCanRouteTo a x ≡ true) b'≡b eq
      helper false eq =
  ...
  private
    ...
    mem-++-split' {x ∷ xs} (hereA y≡x) = inl (hereA y≡x)
